...
class Digest2GUI:
    ...
    def create_evaluation_tab(self):
        ...
        desc_label = ttk.Label(
            eval_frame,
            text="选择文件或输入小行星的 MPC 80列、ADES 格式短弧观测数据，点击\"开始分析\"，程序将自动评估轨道类型。您的数据不会以任何形式存储、保留或记录。",
            wraplength=0
        )
    ...
    def create_description_tab(self):
        ...
        orbit_types = [
            ...
            ('MB1', 'Inner Main Belt', '内主带', '𝑞 > 1.67 AU，2.1 AU < 𝑎 < 2.5 AU，𝑖 < 25𝑎−45.5'),
    ...
    def _load_ades_psv_content(self, filename):
        ...
        # 解析文件获取统计信息（permID优先，provID次之）
        try:
            import csv
            from io import StringIO

            # 重新解析PSV文件，按permID优先统计天体数量
            body_ids = set()
            total_obs = 0
            headers = None
            ...
            status_msg = f"已加载 ADES PSV 文件：{len(body_ids)} 个天体，共 {total_obs} 条观测 - 请直接点击'开始分析'"
        except:
    ...
    def _load_ades_xml_content(self, filename):
        ...
        try:
            ...
            status_msg = f"已加载 ADES XML 文件：{len(body_ids)} 个天体，共 {total_obs} 条观测 - 请直接点击'开始分析'"
        except:
